...
function findIndexByProperty(arr, name) {
  ...
  const foundIndex = arr.findIndex(element => element === name);
    return foundIndex;  
}
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const subArr =arr.slice(startIdx);
  const subArrayString = subArr.join(" ");
...
function getArrayKeys(arr) {
  ...
  const iter = arr.keys();
  const arrayKeys = Array.from(iter);
  return arrayKeys;
}
...
function getPositiveArrayValues(arr) {
  ...
  const arrayValues = Array.from(iter);
...
function removeAndDouble(arr, startIdx) {
  ...
  const arrSmall = arr.splice(startIdx);
  const doubledArray = arrSmall.map(value => value * 2);
  return doubledArray;
}
...
function sumAndPrint(arr) {
  ...
  const sum = arr.reduce((num,user) => num + user, 0);
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const arrMap = arrFlat.map( el => el ** 2);
  const filteredSquaredArray = arrMap.filter((value) => value > threshold);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  const smallArr = arr.slice(-n);
  const sum = smallArr.reduceRight((num,user) => num + user, 0);
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map(elem => 
   elem.length);
  return lengthsArray;
}
...
function insertElementAt(arr, index, element) {
  ...
  const arrSmall = arr.splice(index,0,element);
  return arr;
}
...
//     const expandedArray = arr.flatMap(elem => elem(repeat(n)));
...
function findLongestWord(arr) {
  ...
  const longestWord = arr.reduce((word1, word2) => {
...
function findDuplicateElements(arr) {
  ...
  const duplicateElements = arr.filter((elem1, elem2) => elem1 === elem2);
...
function calculateTotalPrice(arr) {
  ...
  const totalPrice = arr.reduce((price, name) => price + name.price, 0 );
  return totalPrice;
}
...
function findLastNegativeNumberIndex(arr) {
  ...
  const lastIndex = arr.findLastIndex(index => index < 0);
  return lastIndex;
}
